...
func UpdateAvatar(ctx context.Context, req *UpdateAvatarRequest) error {
	uuid := ctx.Value(constant.ReqUuid)
	session := ctx.Value(constant.SessionKey).(string)
	log.Infof("%s|UpdateAvatarRequest access from,user_name=%s|session=%s", uuid, req.UserName, session)
	log.Infof("UpdateAvatarRequest|req==%v", req)

	if session == "" || req.UserName == "" {
		return fmt.Errorf("UpdateAvatarRequest|request params invalid")
	}

	user, err := cache.GetSessionInfo(session)
	if err != nil {
		log.Errorf("%s|Failed to get with session=%s|err =%v", uuid, session, err)
		return fmt.Errorf("UpdateAvatarRequest|GetSessionInfo err:%v", err)
	}

	if user.Name != req.UserName {
		log.Errorf("UpdateAvatarRequest|%s|session info not match with username=%s", uuid, req.UserName)
	}
