...
func UploadAvatar(c *gin.Context) {
	...
	avatarPath := "./web/static/images/"
	c.SaveUploadedFile(file, avatarPath+file.Filename)
	log.Infof("upload avatar success, file name: %s", file.Filename)
	headUrl := "images/" + file.Filename

	// 更新数据库
	session, _ := c.Cookie(constant.SessionKey)
	log.Infof("UpdateAvatar|session=%s", session)
	ctx := context.WithValue(context.Background(), constant.SessionKey, session)
	uuid := utils.Md5String(username + time.Now().GoString())
	ctx = context.WithValue(ctx, "uuid", uuid)
	req := &service.UpdateAvatarRequest{
		UserName: username,
		HeadUrl:  headUrl,
	}
	rsp := &HttpResponse{
	...
	if err := service.UpdateAvatar(ctx, req); err != nil {
		rsp.ResponseWithError(c, CodeUpdateAvatarErr, err.Error())
		return
	}
